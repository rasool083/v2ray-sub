...
def is_config_valid(config_str: str) -> bool:
    ...
    try:
        ...
        # پشتیبانی از پروتکل‌های Hysteria2 و Tuic اضافه شد
        elif config_str.startswith(("trojan://", "ss://", "hysteria2://", "hy2://", "tuic://")):
...
def get_config_signature(config_str: str) -> str:
    """
    برای تشخیص تکراری بودن سرورها، قسمت نام را نادیده می‌گیرد و 
    فقط اتصال اصلی (آی‌پی، پورت و آیدی) را به عنوان امضا برمی‌گرداند.
    """
    try:
        if config_str.startswith("vmess://"):
            b64_part = config_str[8:]
            b64_part += "=" * ((4 - len(b64_part) % 4) % 4)
            data = json.loads(base64.b64decode(b64_part).decode('utf-8'))
            return f"vmess://{data.get('id')}:{data.get('add')}:{data.get('port')}"
        else:
...
def scrape_telegram_source(url: str, source_id: str) -> list:
    configs = []
    current_url = url
    for page_num in range(1, 4):  # کاهش به 3 صفحه برای افزایش سرعت
...
def main():
    ...
    # ۳. منطق جدید حذف تکراری‌ها (فقط مقایسه سرور، نه نام کانفیگ)
